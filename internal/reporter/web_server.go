...
func (ws *WebServer) Start(report *models.Report) error {
	...
	fmt.Printf("🌐 远程访问: http://SERVER_PUBLIC_IP:%d\n", ws.port)
	fmt.Printf("🔐 SSH 转发: ssh -L %d:localhost:%d root@SERVER_PUBLIC_IP\n", ws.port, ws.port)
	fmt.Println("💡 SERVER_PUBLIC_IP 请替换为服务器公网 IP；0.0.0.0 是监听地址，不是浏览器地址")

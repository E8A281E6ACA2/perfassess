...
require_file_contains "$runbook" 'scp root@SERVER_PUBLIC_IP:/tmp/perfassess-auto/calibration_sample\.json'
require_file_contains "$runbook" 'scripts/calibration-collect\.sh ./incoming/vps-standard-001\.calibration_sample\.json calibration-samples'
require_file_contains "$runbook" 'PERFASSESS_CALIBRATION_REQUIRE_MANIFEST=1'
require_file_contains "$runbook" 'manifest\.json'
require_file_contains "$runbook" 'calibration-samples/'
require_file_contains "$runbook" 'docs/calibration-sampling-matrix\.md'
...
require_file_contains "$runbook" 'PERFASSESS_IPERF3_SERVER=192\.0\.2\.10:5201 bash'
require_file_contains "$runbook" '文档保留地址'

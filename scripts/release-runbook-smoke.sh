...
require_file_contains "docs/release-checklist.md" '--require-score-profiles vps,server,workstation --require-policy formal'
require_file_contains "docs/release-checklist.md" 'profile_policies'
require_file_contains "docs/release-checklist.md" 'scripts/calibration-collect\.sh'
require_file_contains "docs/release-checklist.md" 'PERFASSESS_CALIBRATION_REQUIRE_MANIFEST=1'
require_file_contains "docs/release-checklist.md" '不得把原始 `default\.json`、日志、压缩包或完整报告目录作为校准输入'
require_file_contains "$runbook" 'make release-check'
require_file_contains "$runbook" 'scripts/calibration-collect\.sh'
require_file_contains "$runbook" 'PERFASSESS_CALIBRATION_REQUIRE_MANIFEST=1'
require_file_contains "$runbook" '不要把原始报告、日志、压缩包或完整报告目录作为校准输入'
require_file_contains "$runbook" '--require-score-profiles vps,server,workstation --require-policy formal'
require_file_contains "$runbook" 'score_profile'

...
`--web` 会在测评开始前启动实时进度页，浏览器打开 `http://服务器公网IP:8080` 后可看到当前步骤、完成状态和已生成报告文件。脚本中显示的 `0.0.0.0:8080` 只是服务监听所有网卡的地址，不是浏览器访问地址；浏览器应使用服务器公网 IP、内网 IP，或本地 SSH 端口转发地址。如果 8080 已被占用，bootstrap 会自动继续尝试 8081、8082，直到找到可用端口，并在终端打印最终可访问地址。
...
A: 默认从 8080 开始自动探测；如果 8080 被占用，会继续尝试 8081、8082，直到找到可用端口。终端会打印实际访问地址。也可以使用 `--port` 指定起始端口：
```bash
./build/perfassess -b all --web --port 9090
```
